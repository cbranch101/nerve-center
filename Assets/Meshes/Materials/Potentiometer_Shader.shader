...
////// Emissive:
                ...
                float node_55 = 9.0;
                float3 emissive = ((tex2D(_PotentiometerGradient,TRANSFORM_TEX(node_139.rg, _PotentiometerGradient)).rgb*step(tex2D(_Gradient,TRANSFORM_TEX(node_36, _Gradient)).rgb,floor((_BulbsIlluminated/9.0) * node_55) / (node_55 - 1)))*_BulbBrightness);
///////// Gloss:
                float gloss = _BulbSpecular.a;
                float specPow = exp2( gloss * 10.0+1.0);
////// Specular:
                NdotL = max(0.0, NdotL);
                float4 node_2 = tex2D(_PotentiometerGradient,TRANSFORM_TEX(node_139.rg, _PotentiometerGradient));
...
////// Specular:
                ...
                float4 node_2 = tex2D(_PotentiometerGradient,TRANSFORM_TEX(node_140.rg, _PotentiometerGradient));
                float3 specularColor = (node_2.rgb*_BulbSpecular.rgb);
