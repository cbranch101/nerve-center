...
Shader "Custom/Potentiometer_Shader" {
    Properties {
        ...
        _Level ("Level", Range(0, 1)) = 0.5213675
    }
    SubShader {
        Tags {
            "RenderType"="Opaque"
...
////// Emissive:
                ...
                float node_55 = 9.0;
                float3 emissive = ((tex2D(_PotentiometerGradient,TRANSFORM_TEX(node_106.rg, _PotentiometerGradient)).rgb*step(tex2D(_Gradient,TRANSFORM_TEX(node_36, _Gradient)).rgb,floor(_Level * node_55) / (node_55 - 1)))*_BulbBrightness);
///////// Gloss:
