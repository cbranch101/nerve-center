...
/////// Normals:
                ...
                float2 node_14 = (0.05*((tex2D(_DepthMapAlpha,TRANSFORM_TEX(node_67.rg, _DepthMapAlpha)).r*_ParalaxDepth) - _ParalaxReference)*mul(tangentTransform, viewDirection).xy + node_67.rg);
...
/////// Diffuse:
                ...
                float3 diffuse = pow(max( 0.0, NdotL), _Specular.rgb) * attenColor + UNITY_LIGHTMODEL_AMBIENT.xyz;
///////// Gloss:
                float gloss = 0.5;
...
/////// Normals:
                ...
                float2 node_14 = (0.05*((tex2D(_DepthMapAlpha,TRANSFORM_TEX(node_68.rg, _DepthMapAlpha)).r*_ParalaxDepth) - _ParalaxReference)*mul(tangentTransform, viewDirection).xy + node_68.rg);
                float3 normalLocal = UnpackNormal(tex2D(_NormalMap,TRANSFORM_TEX(node_14.rg, _NormalMap))).rgb;
                float3 normalDirection =  normalize(mul( normalLocal, tangentTransform )); // Perturbed normals
